...
Then(/^(?:I|they|"([^"]*?)") should receive (an|no|\d+) emails? with (?:the )?subject "([^"]*?)"$/) do |address, amount, subject|
...
Then(%r{^(?:I|they|"([^"]*?)") should receive (an|no|\d+) emails? with (?:the )?subject /([^"]*?)/$}) do |address, amount, subject|
...
When(/^I open the email sent to "([^"]*)"$/) do |address|
  open_email(address)
end
